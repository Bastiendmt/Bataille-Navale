...
import Outils
import string
import ntpath ##Verifie si un fichier existe
import random
...
def positionsDesBateaux(flotte,exepte): #OP  #Utilisé pour positionneBateauParDirection
    dictbateau={}
    ...
    for elm in listebateau:
        if elm not in exepte :      ##Si un bateau se trouve dans la lsite exepte la boucle ne va pas procéder à l'affichage des coordonnées de ce bateau
            for elm2 in listebateau[elm]:
                #print elm2, elm
                dictbateau[elm2]=elm   ##Associe la coordonee d'un bateau au nom du bateau
    return dictbateau
...
def choixFlotteAleatoire(flotte): #OP
    ...
    flotte= InitialisationdDuneFlotteVide()
    ##print flotte
    for elm in flotte['bateaux']:
        while flotte['bateaux'][elm] == []:          ## tant qu'un bateau n'a pas ete place, sa premiere case et sa direction change
            ...
            positionneBateauParDirection(flotte,elm,case,direction)
    return flotte
...
##print maFlotte
##print saFlotte
##maFlotte=choixFlotteAleatoire(maFlotte)
##saFlotte=choixFlotteAleatoire(saFlotte)
##
##
##
##print maFlotte
##print saFlotte
##print id(maFlotte),id(saFlotte)
...
if __name__ == '__main__' :
    ...
    print 'Cette partie est realisee uniquement lorsque le programme flotte est lance'
